...
export const expensesRoutes: FastifyPluginAsync = async (app) => {
  app.post('/api/expenses', async (req, reply) => {
    ...
    const b = parsed.data;
    const cat = app.db.prepare('SELECT id FROM categories WHERE id = ? AND is_active = 1').get(b.categoryId);
    if (!cat) return reply.code(400).send({ error: 'Категория не найдена' });
    const info = app.db
      .prepare('INSERT INTO expenses (user, date, amount, category_id, comment) VALUES (?, ?, ?, ?, ?)')
      .run(b.user, b.date, b.amount, b.categoryId, b.comment ?? null);
  ...
  app.delete('/api/expenses/:id', async (req, reply) => {
    ...
    const info = app.db.prepare('DELETE FROM expenses WHERE id = ?').run(id.data);
    if (info.changes === 0) return reply.code(404).send({ error: 'Трата не найдена' });
    return { ok: true };
  });
};
