...
export const transfersRoutes: FastifyPluginAsync = async (app) => {
  app.post('/api/transfers', async (req, reply) => {
    ...
    const b = parsed.data;
    const info = app.db
      .prepare('INSERT INTO transfers (from_user, to_user, date, amount, comment) VALUES (?, ?, ?, ?, ?)')
      .run(b.fromUser, b.toUser, b.date, b.amount, b.comment ?? null);
  ...
  app.delete('/api/transfers/:id', async (req, reply) => {
    ...
    const info = app.db.prepare('DELETE FROM transfers WHERE id = ?').run(id.data);
    if (info.changes === 0) return reply.code(404).send({ error: 'Перевод не найден' });
    return { ok: true };
  });
};
